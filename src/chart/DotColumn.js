...
(function (root, factory) {
    if (typeof define === "function" && define.amd) {
        define(["d3", "./XYAxis", "../api/INDChart", "../api/ITooltip", "css!./DotColumn"], factory);
    } else {
    ...
    DotColumn.prototype.updateChart = function (domNode, element, margin, width, height, isHorizontal, duration) {
        var context = this;
        ...
        if (!this.graphLayer) {
            ...
            this.inputLayer = this.svgData.append('g');
        }
        ...
        function trans(to) {
            ...
            context.inputLayer.selectAll('.button rect')
                .style('fill', function (d, i) { return (i === time) ? 'rgba(0,0,0,0.9)' : 'rgba(0,0,0,0.1)'; })
            ;
        ...
        years = this.columns().filter(function (col, idx) { return idx > 0; });
        var yearIdx = 0;
        var yearTarget = years[yearIdx];
        var parties = this.data().map(function (row) { return row[0]; });
        var partDict = {};
